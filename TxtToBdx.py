...
class MCCmd2Bdx:
    ...
    class CmdArrays:
        ...
        def append(
            self,
            cmd: str,
            cb_mode=bdx_work_shop.canvas.Canvas.MODE_CHAIN_CB,
            delay=0,
            conditional=False,
            needRedstone=False,
        ):
    ...
    def dump(self, file_path="test.bdx"):
        ir = self.canvas.done()
        bdx_work_shop.canvas.irio.dump_ir_to_bdx(
            ir, file_path, need_sign=False, author="SuperBdxSpawner"
        )
    ...
    def input_cmds_array(
        ...
        if not auto_cond:
            for cmd, mode, delay, conditional in cmd_array.array:
                ...
                self.put_cmd_block(
                    now_x, now_y, now_z, cmd, facing, mode, delay, conditional
                )
            ...
            try:
                ...
                match dir2:
                    ...
                    case "z-":
                        dir2 = canva.FACE_ZNN
                    case _:
                        raise Exception("初始方向1不正确: 必须为 x+/x-/z+/z-")
            ...
            if "#" in cmd:
                try:
                    ...
                    if "(" in cmd:
                        ...
                        match next_mode_zh:
                            case "脉冲":
                                next_mode = canva.MODE_CB
                            case "连锁":
                                next_mode = canva.MODE_CHAIN_CB
                            case "循环":
                                next_mode = canva.MODE_REPEAT_CB
                            case "":
                                pass
                            case _:
                                pass
...
def test_bdx(path: str):
    ...
    op = MCCmd2Bdx()
    op.snake_folding_cmds(arr, first_dir, sec_dir, max_len).dump(
        path.replace("mcfunction", "bdx")
    )
