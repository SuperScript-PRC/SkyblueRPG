...
from . import define as fishing_define, event_apis

reload(fishing_define)
reload(event_apis)

from .define import Cube, Bait, FishingArea

FISH_PERCENT = 0.95


class CustomRPGFishing(Plugin):
    name = "自定义RPG-钓鱼系统"
    author = "SuperScript"
    version = (0, 0, 1)
    ...
    def hook_ok_give(self, player: Player, area: FishingArea):
        ...
        if random.random() >= hook_percent:
            self.BroadcastEvent(
                event_apis.PlayerFishHookedEvent(player, [], False).to_broadcast()
            )
            player.show(
                "§6┃ ☬ 鱼儿脱钩啦... 这是一杆空竿。",
            )
        ...
        hooked_item = self.rpg.item_holder.createItems(final_section)
        self.BroadcastEvent(
...
entry = plugin_entry(CustomRPGFishing, "自定义RPG-钓鱼")
