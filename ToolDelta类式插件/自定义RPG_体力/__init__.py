...
class CustomRPGPower(Plugin):
    ...
    def check_source_get(self, player: Player):
        ...
        if final_power_getted > 0:
            self.rpg.backpack_holder.addPlayerStore(
                player,
                self.rpg.item_holder.createItem(self.power_item, final_power_getted),
            )
            ...
            self.playerdata.set_property(player, "crpg:last_get_power", current_time)
